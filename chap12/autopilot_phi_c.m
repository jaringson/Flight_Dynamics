...
%     chi_c    = uu(3+NN);  % commanded course (rad)
    chi_c = 0; % not using chi_c in equations right now.
    phi_c    = uu(3+NN);  % commanded course (rad)
%    phi_c_ff = uu(4+NN);  % feedforward roll command (rad)
    ...
    switch autopilot_version
        ...
        case 2,
           [delta, x_command] = autopilot_uavbook(Va_c,h_c,phi_c,phi_c_ff,Va,h,chi,phi,theta,p,q,r,t,P);
        case 3,
...
function [delta, x_command] = autopilot_tuning(Va_c,h_c,chi_c,Va,h,chi,phi,theta,p,q,r,t,P)
    ...
    switch mode
        case 1, % tune the roll loop
            phi_c = phi_c; % chi_c; % interpret chi_c to autopilot as course command
        ...
        case 2, % tune the course loop
            if t==0,
                phi_c   = phi_c; % course_hold(chi_c, chi, r, 1, P);
            else
                phi_c   = phi_c; % course_hold(chi_c, chi, r, 0, P);
            end                
            delta_a = roll_hold(phi_c, phi, p, P);
            delta_r = 0; % no rudder
            % use trim values for elevator and throttle while tuning the lateral autopilot
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [delta, x_command] = autopilot_uavbook(Va_c,h_c,phi_c,phi_c_ff,Va,h,chi,phi,theta,p,q,r,t,P)

    %----------------------------------------------------------
    % lateral autopilot
    if t==0,
        phi_c   = phi_c_ff+phi_c; % course_hold(chi_c, chi, r, 1, P);
        % assume no rudder, therefore set delta_r=0
        delta_r = 0;%coordinated_turn_hold(beta, 1, P);
    else
        phi_c   = phi_c_ff+phi_c; % course_hold(chi_c, chi, r, 0, P);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [delta, x_command] = autopilot_TECS(Va_c,h_c,phi_c,phi_c_ff,Va,h,chi,phi,theta,p,q,r,t,P)
...
%         phi_c   = sat(phi_c_ff+course_hold(chi_c, chi, r, 0, P),P.phi_max,-P.phi_max);
        phi_c   = sat(phi_c_ff+phi_c,P.phi_max,-P.phi_max);
        delta_r = 0;%coordinated_turn_hold(beta, 0, P);
    end
    delta_a = roll_hold(phi_c, phi, p, P);     
