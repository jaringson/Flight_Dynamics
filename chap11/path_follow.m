...
function out = path_follow(in,P)
  ...
  switch flag,
      case 1, % follow straight line path specified by r and q
          ...
          epy = -sin(chi_q)*(pn - r_path(1)) + cos(chi_q) *...
                (pe - r_path(2));
      ...
      case 2, % follow orbit specified by c, rho, lam
          ...
          bar_phi = atan2(pe-c_orbit(2),pn-c_orbit(1));
          while bar_phi - chi < -pi
             bar_phi = bar_phi + 2*pi; 
          end
          while bar_phi - chi > pi
             bar_phi = bar_phi - 2*pi; 
          end
          ...
          chi_c = bar_phi + lam_orbit*((pi/2)+atan(k_orbit*(d-rho_orbit)/rho_orbit));
